...
from ..core.contracts import (
    Axis, Confidence, Finding, RunContext, Scope, Severity)
...
def stage_services(run: RunState, cfg: dict) -> CheckpointResult:
    ...
    return CheckpointResult(State.OK, outputs={
        ...
        "finding_count": n,
    }, findings=n, note=(f"{len(services)} started by config; web: "
             f"{', '.join(web) or 'none'}"
...
def stage_triage(run: RunState, cfg: dict) -> CheckpointResult:
    ...
    for stage in ("secrets", "elfscan", "services",
...
def stage_report(run: RunState, cfg: dict) -> CheckpointResult:
    ...
    return CheckpointResult(State.OK, outputs={"path": str(run.root / "report.json")},
                            note=f"{len(findings)} findings written")
...
def build_pipeline() -> Pipeline:
    return Pipeline([
        ...
        Checkpoint("triage", "Dedupe and verify findings", stage_triage,
                   needs=["secrets", "elfscan", "services",
                          "reachability", "webscan"]),
