...
def build_runtime_proof(result: DifferentialResult, ctx: RunContext,
    ...
    # control_b belongs in the CLAIM, not merely in the blob store. The
    # second control is what distinguishes a real divergence from a noisy
    # endpoint, and a verifier that cannot see it re-checks only half the
    # oracle -- which is the half that was never in doubt.
    claim: dict = {"control_transcript": ctrl,
    ...
    if result.dimension == "marker":
        ...
        # The dimension that actually diverged. Hardcoding "status" made the
        # verifier re-check a dimension the detector never measured.
